...
df.dropna(subset=['timestamp'], inplace=True)  # Drop rows where timestamp could not be parsed
...
# Binary label for ground truth anomalies (assuming Severity_level > 1 indicates an anomaly)
df['true_anomaly'] = df['Severity_level'].apply(lambda x: 1 if x > 1 else 0)
...
print(classification_report(y_true, y_pred_lof, zero_division=0))
